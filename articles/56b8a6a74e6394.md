...
`--features pcre2` をつけろとドキュメントに書いてあったので `cargo install ripgrep -- --features pcre2` みたいなことをしたけど効かなかった。しょうがないのでソースのところでビルドしてコピった。常識的に考えて cargo install のときにオプションを渡す方法があるんだと思うけどわからなかった。
...
| --crlf             | 改行が `\r\n` なときでも `$` が行末にマッチするようになる |
  ...
  * 結局 Windows なファイルには `-E shift_jis --crlf` としないといけない
    * それか --pre のコマンドで `nkf -w` する？ (のはやめたほうがよさそう)
