...
ブラウザ上で行う面倒な作業を自動化しているうちにいろんなノウハウが溜まったのでまとめた。

もともと Capybara はテスト用のフレームワークで、ブラウザ操作は selenium-webdriver の役割だが、Capybara から扱う方がブラウザ操作もやりやすかったので、ほぼ Capybara を扱う内容になっている。
...
## 連続する要素の指定番目の要素を拾うのに all を使ってはならない
...
- `:field` はすべての入力タグが対象になるけど `:fillable_field` だとテキスト関連に絞られる
- `input[type="text"]` と textarea だけが対象かと思ったが `input[type="color]` なども含まれる
- type は text date datetime datetime-local month week time number range search tel url email color が対象
- なので `find(:field, type: "text")` とするぐらいなら `find(:fillable_field)` とした方がよい
- 他はとくにメリットはないけど「文字列を入力している」ことをソースコード上で明示できる
...
```ruby
Capybara.find("#foo").value                            # => "2"
Capybara.find(:id, "foo").value                        # => "2"
Capybara.find(:field, id: "foo").value                 # => "2"
```

表示名を得るには :checked で option タグを find する必要がある

```ruby
Capybara.find("#foo :checked").text                    # => "B"
Capybara.find("#foo").find(":checked").text            # => "B"
Capybara.find(:id, "foo").find(":checked").text        # => "B"
Capybara.find(:field, id: "foo").find(":checked").text # => "B"
```

- このようにいろんな方法があるが一意の ID 指定であっても、何の ID かわかりやすくするため、なるべく `:field` セレクタを使った方がよい
...
とすれば良いことに気づいて多用していたのだけど、これをすると本来普通に選択できるはずの要素が**選択できなくなる**という罠がある。

しかもエラーの原因が `document.body.style.zoom` に起因するものであることが非常にわかりにくくてめちゃくちゃはまる。
