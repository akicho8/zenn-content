...
## 登録済みのセレクターを使った方が良い理由

次のタグは disabled なので押せない

```html
<button disabled>送信</button>
```
  
だけど find + CSS セレクターを使うと取得できる。その上(見かけ上は)押せてしまう。
...
もちろんサイト側では何も起きない。
本来は選択できてはいけないので disabled の逆を意味する enabled を指定しないといけない。  
