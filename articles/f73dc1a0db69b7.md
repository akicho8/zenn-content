...
Capybara でブラウザ操作を半自動化する際に得た泥臭い知見をまとめています。
...
- ただ昔のことで記憶が怪しいけど、もしかしたら brew の chromedriver と chromedriver-helper gem と webdrivers gem が何をするものか正確に把握できておらず webdrivers gem を正しく使えてなかったのかもしれない。もしそうなら webdrivers にはすまん
...
webdrivers は Google Chrome のバージョンに合わせた chromedriver を用意してくれるはずなので ~/.webdrivers をバックアップする必要はないのかもしれないが、信用できないほどに事故を恐れているので、こちらもしっかり管理下に入れてすぐ戻せるようにしておく。
...
- capybara が古いのが原因
- どこで直ったのかはわからんけど 3.37.1 以上だと警告は出なくなっていた
...
`Capybara.visit(url)` だと戻値もわからないので代わりに faraday を使う
...
## default_selector が :css なのに xpath の表記もできてしまうのはなぜか？

Nokogiri が融通を効かせているから？
...
`LOOKS_LIKE_XPATH` にマッチすると xpath と見なす
...
つまり `/` で始まっているから xpath と見なされたようだ (続く)
...
- `text()='xxx'` は問題ない
- しかし `contains(text(), 'x')` を書くと失敗する
- `CSS::SyntaxError` とあるので CSS として見ている？
- この場合は `:xpath` を明示すると通った

```ruby
node.has_selector?(:xpath, "//a[contains(text(), 'x')]")   # => true
```

- Nokogiri 側では問題ないので Capybara でも似たようなことをやろうとしてうまくいっていないようだ
...
## グローバルで `exact = true` にしてしまうと assert_text まで完全一致モードになる？
...
- 複雑なコンポーネントの場合、行儀良く a や button だけが click に反応するようには設計されていない
- そういう場合はその文言を持ったタグを click すれば伝搬する
