...
上だけ見てもわからないが、内部では次のように `a` を `//a` に変換してから探している。
...
1. CSS として `//a` が入力された (そんなCSS表記ない)
1. `////a` に変換された (そんなセレクタもない)
1. しかし、引けた (なんで？)
...
- この問題のやっかいなのは似たセレクタの一方は通るところ
  - 完全一致は通って部分一致は通らない
- 両方通らなければ CSS に XPath を指定したミスにすぐ気づいた
- しかし、完全一致のセレクタは通るので、失敗する原因がわからない
- 原因は部分一致の書き方にあると考えてしまい、ますます深みにはまっていった
  - 実際4年ぐらいはまっていた
  - XPath 恐怖症になっていた
- 思うに `Nokogiri::CSS.xpath_for` の仕様がよくない
  - CSS として XPath が入力されたらエラーとすべきだ

結論
- XPath を書くときは **必ず XPath を明示** する

こんなん書いて動いているのはたまたま。置き換える。
