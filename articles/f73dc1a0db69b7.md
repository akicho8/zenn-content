...
そういうときはパスではなくコンテンツに含まれるものが現われるまで待つようにする。
...
指定したメソッドだけ
```ruby
Capybara.assert_title("Google", wait: 5)
```
...
## display:none なタグを選択する

実際に目で見えないタグは選択できないが `visible: :all` をつけると選択できる
