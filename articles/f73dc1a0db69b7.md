...
- 内部では 0.01 秒間隔で実行している
- ということは1秒間で100回実行している
- `wait: 60` とした場合タイムアウトまでに 6000 回実行することになる
- それが原因
- 次のようにすると500分の1の処理量になりPCは怒らなくなる

```ruby
100.times do
  second = 5 # 5秒間に1回だけ確認する
  (60 / second).times do |i|
    if Capybara.has_selector?("button", text: "送信", exact_text: true, wait: 0)
      success = true
      break
    end
    sleep(second)
...
Other element would receive the click: <div class="SpinnerBackdrop"></div>
```

### 対策1. 消えるまで待つ

```ruby
Capybara.assert_no_selector("[class*=Spinner]", wait: 60)
```
...
```ruby
Capybara.execute_script("document.querySelector('.SpinnerBackdrop').remove()")
```
