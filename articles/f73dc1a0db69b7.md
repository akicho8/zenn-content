...
タグを指定しないなら組み込みセレクタの `:element` を使う方法もあるけど `html` タグからマッチしてしまう
...
これを自動的にやってくれるのが登録済みの `:button` セレクタになる。
...
## 組み込みの :element セレクタは disabled を考慮してくれない
