...
とりあえず動けばいいではなく常に完璧なセレクタを書くように心掛ける。
そのための検証・練習にとても役立つ。
...
## 連続する要素の指定番目の要素を拾うのに all は使ってはならない
...
## なるべく組み込みセレクターを使った方が良い理由

セレクターに `"button"` を使うより `:button` を使った方が良い理由は `disabled` 属性を考慮してくれるから。
...
だけど find + CSS セレクターを使うと取得できてしまう。その上(見かけ上は)押せてしまう。
...
これを自動的にやってくれるのが登録済みの `:button` セレクターになる。
...
## 組み込みセレクターを使いつつ disabled なタグも選択したい
...
## 組み込みセレクターだとマッチしすぎるけど必ずフィルタする方法がある
