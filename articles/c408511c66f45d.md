...
```
  ...
  * cargo は Ruby でいうところの gem コマンドと bundler コマンドを混ぜたようなもの
  * linux では apt 等で fd だけをインストールできる
  * macOS では `brew install fd` でも fd だけをインストールできる
  * でも普段の開発環境であれば cargo 経由のインストールの方がおすすめ
...
#### マッチしてないのにステイタス0を返してしまう ####
  ...
  * マッチの有無に関わらず指定しないと常に 0 のようだ
  * このあたり作者の思想が違うためか微妙に ripgrep と仕様が異なる
  * できれば合わせてほしい

# 実用例 #

#### XMLファイルはGit管理下にあるけど検索対象から除外したい ####

```shell
echo "*.xml" >> .fdignore
```

#### 除外するルールは rg (ripgrep) と共有したい ####
