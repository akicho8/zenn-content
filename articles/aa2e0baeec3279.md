...
```
  ...
  * しかし、そのメリットよりマージした際にキーが別々になってはまることの方が多い
...
二つの機能 foo と bar は互いに何も関係ないにもかかわらず、同じスコープで共存しているのが失敗だった
...
```
  ...
  * 一つのコードにまとめたが実際はモジュールやクラスはさらにファイルに分割する
    ...
    * foo/commands.rb
    * foo/controller.rb
  * Thor 継承クラスはあくまでエントリーポイントとしての利用に留める
