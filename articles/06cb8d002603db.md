...
transient.el は Magit で使われているメニュー表示用のライブラリです
...
```
  ...
  * `-x` をタイプすると `--xxx` が有効になる (わかりにくいけど色が少し変わる)
...
となるので第二引数にはオプションの配列 `(transient-args 'f1)` を渡せばいい
...
(transient-define-prefix f1 ()
  [
   ...
   ("c" "item3" c3 :if-mode (org-mode emacs-lisp-mode))
   ])
```
...
横2列で表示するとき項目名に日本語を使うとエラーになる場合がある
そんなときは `:variable-pitch t` を指定すると回避できる
...
これは個別に指定するよりグローバルな設定とした方がよさそう

```emacs-lisp
(setq transient-align-variable-pitch t)
```

次の固定幅フォント使うオプションを t にしても回避できるけど上ので十分だと思う
