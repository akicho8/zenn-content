...
本家では `add` で `adder` と `add-one` を作る例になっていたため名前で混乱しました。その上 `add` 自体がさすがにワークスペース名なわけはないだろうと先入観を持ってしまいました。なのでドキュメントには書いてないけどあらかじめワークスペースとなるその親ディレクトリを `cargo new` で用意しとかないといけないのかな？ と考えてしまってディレクトリ構造を深くしてしまったり、`add` 自体を `cargo new` で作ってしまったりと無駄な試行錯誤を重ねてしまいました。
