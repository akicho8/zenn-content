...
ところで Window は create_renderer のタイミングでいきなり起動するわけじゃないみたい。

`SDL2::Event.poll` を呼び続けて次のイベントを順に受信し、
...
環境によって違うのかもしれないけど、私のところでは7ループ目に発生した。

なので Window を2秒間表示するというなら正確には次のように `FOCUS_GAINED` を待ってから `sleep(2)` するのが正しい。
