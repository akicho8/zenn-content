...
Ruby/SDL は更新されないものと諦めていたらひっそりと [Ruby/SDL2](https://rubygems.org/gems/ruby-sdl2) が公開されていたので使ってみる。
...
上のシェルスクリプトを流せば次のようにウィンドウが2秒表示される。

![window](/images/7170a5dc60c97b/window.png)

ちなみに最後のところのループを次のように 5 回にしたら Window が開かず 6 回にしたら開くっていう挙動は謎。
...
リポジトリのサンプルでは `renderer.present` を呼ぶだけで Window が起動する想定になっているのになんでだろう？ (誰か教えて)
...
──として呼び出し順序と必要最低限な処理だけを書いとく。
抽象クラスだけどウィンドウが出ないアプリとして実行できる。
...
マジックナンバーが目立つけどほっとく。抽象化したり定数化したくなってくるけど逆にやらない我慢が大切で、やるメリットがめちゃくちゃある場合にやっとやるかやらないかぐらいでよい。
...
ぱっと見、ミニフレームワーク内によくわからないインスタンス変数がばらまかれたのがいまいちなのでクラス化した方がよい気がするけど、それはいったん置いとく。
...
view を2回定義しても順番に呼ばれたのがわかる。
ただし super を呼ばずにオーバーライドする余地がなくなるので一長一短ある。
...
できた。上のようにオーバライドしたので次のように書けるようになった。
...
[Tixy](https://tixy.land/) (Creator: [@aemkei](https://twitter.com/aemkei)) は短いコードでクリエイティブコーディングできるクールなサイトだ。
...
- `tixy_func` 関数の中身を変えるといろんな模様を表現できる
- 16 * 16 = 256 個の fill_rect をしても60フレームでぬるぬる動いている
