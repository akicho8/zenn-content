...
Window は create_renderer のタイミングで起動しているわけじゃないみたい。

まず `SDL2::Event.poll` を呼び続け、溜まったイベントを捨てないといくら待っても起動しない。
...
なので Window を2秒間表示するというならば正確には次のように `FOCUS_GAINED` を待ってからにしないとおかしい。が、そんな些細なことをつっこむ人は誰もいないだろう。
