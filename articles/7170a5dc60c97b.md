...
ところで Window は create_renderer のタイミングでいきなり起動しているわけじゃないみたい。

まず `SDL2::Event.poll` を呼び続け、溜まったイベントを受け流さないといくら待っても起動しない。
...
なので Window を2秒間表示するというならば正確には次のように `FOCUS_GAINED` を待ってからにしないとおかしい。が、そんな些細なことは気にしなくてよかろう。
...
*24 FPS の GIF なのでカクカクしているけど実際はぬるぬるしている*
