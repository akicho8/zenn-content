...
サンプルとしてはベタ書きの方が一直線に処理が追えてわかりやすいのですが、そのままコードを書き足していくと何がなんだかわからなくなってきます。
...
抽象クラスですが実行するとウィンドウが出ないアプリがとりあえず動きます。
...
クリエイティブコーディング系は特殊でマジックナンバーだらけになりがちです。
...
いま何FPSで動いているのか知りたかったら次のようなモジュールを追加します。
...
テキスト表示するだけでこんなに書かないといけないのかって感じですが使いやすいメソッドにしてしまえば問題ないでしょう。

左上にカウンタと秒間フレーム数を表示できました。

![font](/images/7170a5dc60c97b/font.png)

### super だらけなのが気になる？

場合によっては super を呼ばないときもあるので仕方ないとはいえ、super を呼ばないときはないし、逆に super を書き忘れてはまることの方が多いってときは Rails の before_action のような仕組みにすると良いかもしれないです。
