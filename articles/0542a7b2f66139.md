---
title: "Nannouのサンプルを参考してテンプレートを作ってみた"
emoji: "🐣"
...
Nannou であれこれ試すときに公式のサンプルを参考にしながらいろいろ追加していくのが手間だったのでテンプレートを作りました。
とりあえずこれを元にして、いらないのは消していく感じです。
...
## 初めての方へのNannouの導入と実行手順

`~/src/nannou_playground` のなかに作るとします。
...
として examples のなかにファイルを置いていきます。
そこで `examples/template.rs` を実行するには `cargo run --example template` とします。
`src/main.rs` の方に書いてもいいんですが、いくつもファイルを作っていろいろ試すときは examples の中に作ると切り替えやすいです。
