...
*24 FPS の GIF なのでカクカクしてるけど実際はぬるぬる*

## はじめに

追加していくより消す方が簡単なので全部入りをテンプレートを最初に作っておく。
...
そこで `examples/template.rs` を実行するには `cargo run --example template` とする。
`src/main.rs` に書いてもいいけど、いろいろ試すときは examples の中に作ると切り替えやすい。
